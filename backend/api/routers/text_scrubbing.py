...
def descrub(
    ...
    audit_log = audit_manager.get_log(req.scrub_id)
    if not audit_log:
    ...
    if req.descrub_all:
        original_text = audit_log.details.get("original_text", "")
        ...
        entities = audit_log.details.get("entities", [])
        if not entities:
            raise HTTPException(
                status_code=400, detail="No entities found in scrub record"
            )

        scrubbed_text = audit_log.details.get("anonymized_text", "")
