...
API_URL = os.environ.get("SECUREPROMPT_API", "http://127.0.0.1:8000/api/v1")
...
if "token" not in st.session_state:
    ...
    with st.form("login_form"):
        ...
        CorpKey = st.text_input("CorpKey", type="password")
        submit = st.form_submit_button("Log In")

    # Debugging the login response
    if submit:
        response = requests.post(f"{API_URL}/login", json={"email": email, "CorpKey": CorpKey}, timeout=10)
        st.write("API Response:", response.json())  # Debugging line
        if response.status_code == 200:
            data = response.json()
            st.session_state["token"] = data["token"]
            st.success(
                f"Welcome, {data['first_name']} {data['last_name']} ({data['role']})!"
            )
            st.rerun()
        else:
    ...
    with col2:
        ...
        with right:
            if st.button("➡️ Logout", type="tertiary"):
                ...
                r = requests.post(f"{API_URL}/logout", headers=headers, timeout=10)
